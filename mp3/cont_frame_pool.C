...
ContFramePool::FrameState ContFramePool::get_state(unsigned long frame_no) {
    ...
    if ((bitmap[bitmap_index] & mask) == 0b11) {
        state_output = FrameState::Used;
    } else if ((bitmap[bitmap_index] & mask) == 0b00) {
        state_output = FrameState::Free;
    } else if ((bitmap[bitmap_index] & mask) == 0b01) {
...
void ContFramePool::set_state(unsigned long frame_no, FrameState framestate) {
    ...
    case FrameState::Used:
        bitmap[bitmap_index] = bitmap[bitmap_index] | mask;  //consider bitmap is 0011 => 0011 | xxxx => xx11 (Used)
        break;
    case FrameState::Free:
        bitmap[bitmap_index] = bitmap[bitmap_index] & ~mask; //consider bitmap is 0011 => ~0011 & xxxx => xx00 (Free)
        break;
    case FrameState::HoS:
        bitmap[bitmap_index] = bitmap[bitmap_index] & ~mask; //consider bitmap is 0011 => ~0011 & xxxx => xx00
        mask = 0x1 << position;
        bitmap[bitmap_index] = bitmap[bitmap_index] | mask; //Here bitmap is 0001 => 0001 | xx00 => xx01 (HoS)
        break;
    default:
        break;
    }
...
void ContFramePool::release_frames(unsigned long _first_frame_no)
{
    ...
    while(curr_pool != nullptr) {
    	...
    	curr_pool = curr_pool -> frame_pool_next;
    }
